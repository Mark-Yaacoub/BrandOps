...
export default function NewSalePage() {
  ...
  useEffect(() => {
    if (formData.productId && productsData?.data) {
      const selectedProduct = productsData.data.find((p) => p.id.toString() === formData.productId);
      if (selectedProduct) {
        setFormData((prev) => ({ ...prev, unitPrice: selectedProduct.price.toString() }));
      }
    }
  }, [formData.productId, productsData]);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Add a new sale transaction</p>
      </div>
